...
def zap() -> ZAPv2:
    ...
    try:
        ...
        logger.warning("Could not install ZAP Replacer rule (ZAP may lack the add-on): %s", exc)
    ...
    def test_zap_active_scan_finds_no_critical_access_control_issues(self, zap: ZAPv2, zap_context: dict) -> None:
        ...
        while time.time() < deadline:
            try:
                ...
                logger.warning(
                    "ZAP connection lost during active scan; proceeding with partial results. Error: %s",
                    exc,
                )
                break
            ...
            logger.warning("Active scan exceeded 15 min cap; checking alerts with partial results")
